...
function MyKidSection({ kid }: MyKidSectionProps) {
  const mockKid: KidInfo = kid ?? {
    id: 1,
    name: "김채현",
  ...
  return (
    <section className="pb-8">
      ...
      <div className="mt-6 rounded-[20px] border border-grey-5 bg-white px-5 py-5">
        <div className="flex items-center gap-5">
          <div className="relative h-[68px] w-[68px] shrink-0 overflow-hidden rounded-full bg-grey-4">
            <Image
              src="/svg/ic_child.svg"
              alt={`${mockKid.name} 프로필`}
              fill
              className="object-cover"
            />
          </div>
          <p className="text-body-16-m">{mockKid.name}</p>
        </div>
        ...
        <div className="space-y-8">
          <InfoRow
            title="이번 달 보낸 용돈"
            description={formatMoney(mockKid.monthlyAllowance)}
            actionLabel="용돈지급"
            href={`/kid/${mockKid.id}/allowance`}
          />
          ...
          <InfoRow
            ...
            description={mockKid.walletBalanceText}
          />

          <InfoRow
            title="정기용돈"
            description={mockKid.regularAllowanceText}
            actionLabel="등록하기"
            href={`/kid/${mockKid.id}/regular-allowance`}
            preserveLineBreak
          />
        </div>
        ...
        <InfoRow
          title="용돈 계획"
          description={mockKid.allowancePlanText}
          actionLabel="내역보기"
          href={`/kid/${mockKid.id}/allowance-plan`}
        />
        ...
        <div className="space-y-6">
          <ArrowLinkRow
            label="아이 계좌 추가하기"
            href={`/kid/${mockKid.id}/account/add`}
          />
          <ArrowLinkRow
            label="아이 계좌번호 요청하기"
            href={`/kid/${mockKid.id}/account/request`}
          />
        </div>
      </div>
      ...
      >
        <Plus size={20} className="text-brand-purple-1 mr-5" />내 아이 추가하기
      </Link>
    </section>
  );
}
  ...
  return (
    <div className="flex items-start justify-between gap-4">
      <div className="min-w-0">
        ...
        <p className="mt-1 text-body-16-m text-grey-6 whitespace-pre-line">
          {description}
        </p>
      </div>
